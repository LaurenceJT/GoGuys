...
func PlaylistPlay(playlist Playlist) (beep.Streamer, chan struct{}) {
	...
	song = filepath.Join("playlists", playlist.Name, tracks[trackIndexer].name)
	...
	if len(tracks) == 0 {
		log.Println("No songs in playlist")
		return nil, nil
	}
...
func Pause() {
	if ctrl == nil {
		log.Println("Pause called before initialization")
		return
	}
	speaker.Lock()
...
func Volume(v float64) {
	if vol == nil {
		log.Println("Volume adjustment skipped (not initialized)")
		return
	}
